...
export default class LogHelper {
    static singleInstance: LogHelper = null;
    static getInstance(): LogHelper {
        if (LogHelper.singleInstance == null) {
            LogHelper.singleInstance = new LogHelper();
        }
        return LogHelper.singleInstance;
    }
    // 普通输出
    static log(msg: string) {
        if (GameConfig.logEnable === false) {
            return;
        }
